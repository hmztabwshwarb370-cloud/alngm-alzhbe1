...
async function savePlayer(e){
  ...
  const player = await api('/api/players', { method:'POST', body:JSON.stringify(payload) });
  await loadData(); toast('تم حفظ اللاعب داخل Google Sheets','success');
  $('playerForm').reset(); $('pDate').value = today();
  renderPlayersTable();
  toast('تمت إضافة اللاعب. اضغط زر الطباعة من السجل لإظهار البطاقة.','success');
}
...
async function savePayment(e){
  e.preventDefault();
  try{
    const pay = await api('/api/payments', { method:'POST', body:JSON.stringify({ playerId:$('payPlayer').value, amount:$('payAmount').value, type:$('payType').value, paymentDate:$('payDate').value }) });
    await loadData(); toast('تم حفظ الدفعة داخل Excel','success'); openModal(receipt(pay));
